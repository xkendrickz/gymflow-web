...
async function cetak() {
	...
	loading.value = true
	try {
		const res = await api.get(`https://gymflow-api-production.up.railway.app/api/laporanKinerjaInstruktur/${form.bulan}/${form.tahun}`)
      ...
      <div class="card">
        ...
        <table>
          <tr><th>Nama</th><th>Jumlah Hadir</th><th>Jumlah Libur</th><th>Waktu Terlambat (detik)</th></tr>
          ${data.map((item: any) => `<tr><td>${item.nama_instruktur}</td><td>${item.jumlah_hadir}</td><td>${item.jumlah_libur}</td><td>${item.waktu_terlambat}</td></tr>`).join('')}
